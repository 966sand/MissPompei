...
// 2026-09-18 换名后首句改动：新名自带「地道英语」，原口号首句也是「地道英语」→ 连读成复读机，
// 首句改为「母语者的说法」（同时正是产品主张：母语者真正会说的那句）。**与小程序必须逐字相同**。
function shareTitle() {
  return '母语者的说法，就用地道英语口语助手';
}
...
function resultCaption() {
  ...
  if (!words.length) return '地道英语口语助手：近义词辨析';
  const core = words.length <= 2 ? words.join('、') : (words.slice(0, 2).join('、') + '等');
  return '地道英语口语助手：' + core + '的差异';
}
...
// 圆角 logo：纯渐变方块，不落单字。
// 原先方块里写死白色「词」字（「单词助手」时期字形），与紧邻的「地道英语口语助手」自相矛盾；
...
function drawShareCard(ctx, W, H) {
  ...
  ctx.fillStyle = '#ffffff'; ctx.font = 'bold 19px sans-serif'; ctx.fillText('地道英语口语助手', 70, 42);
  ...
  ctx.fillText('微信搜索「地道英语口语助手」体验完整辨析', 20, H - 18);
}
